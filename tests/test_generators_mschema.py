...
from src.struct_agent.generators.mschema import MSchemaGenerator
...
def test_generate_schema_success(monkeypatch, tmp_path):
    ...
    with patch("src.struct_agent.generators.mschema.SchemaEngine", return_value=fake_engine):
        ...
        assert result is None or isinstance(result, str) or result is None
        fake_schema.save.assert_called()
        assert gen.mschema_str == "{}"
