...
# class MSchemaToMDLConverter:
#     """Converts mschema JSON format to Wren.ai MDL format"""
    
#     def __init__(self, catalog: str = "my_catalog", schema: str = "my_schema"):
...
#             "refSql": f"select * from {db_id}.{table_name}",
#             "columns": []
#         }
...
class MSchemaToMDLConverter:
    ...
    def __init__(self, catalog: str = None, schema: str = "public", env_file: str = None):
        # Load from .env if catalog not provided
        if catalog is None:
            if env_file:
    ...
    @classmethod
    def from_env(cls, env_file: str = '.env', schema: str = "public"):
        """Create converter instance using database name from .env file"""
        load_dotenv(env_file)
        postgres_db = os.getenv('POSTGRES_DB')
        if not postgres_db:
            raise ValueError(f"POSTGRES_DB not found in {env_file}")
        return cls(catalog=postgres_db, schema=schema)
